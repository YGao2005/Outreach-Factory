...
- **Severity / Likelihood:** 3 / 3
- **Owner:** unassigned (Pillar F / E seam)
- **Status:** Open
...
- **Mitigation plan:**
  - Provider adapter in `orchestrator/llm/` (Pillar F precursor). All skills route through it.
...
- **Status:** Mitigated by design (ADR-0038 D179's deterministic-clock contract + the existing `RECENT_BIAS` per-year multiplier per `voice_retrieve.py:113` + the per-register fidelity-score tracking at Week 8); Pillar I drift-detection doctor extension deferred
...
- **Status:** Mitigated by design (ADR-0038 D179's default-to-local `BAAI/bge-small-en-v1.5` model + operator-opt-in cloud model choice + R025-aware cap rule template at Week 2+ deferred)
...
- **Status:** Mitigated by design (ADR-0038 D179's metadata-mismatch refuse-loud — embed_model + embed_version + sentence-transformers version + schema_version are pinned in the cache's sidecar `metadata.json`; the retrieval primitive refuses-loud + auto-rebuilds when allowed)
...
- **Status:** Mitigated by design (ADR-0047 D245's factory IS the only sanctioned construction surface + ADR-0010 D17's `_emitted_by` audit marker + ADR-0047 D249's SKILL.md Phase 6 narrative + ADR-0038 D180's Week 12 Layer 5 reconcile Pass C as the final structural backstop)
