"""Tests for policy migration 0002 — add ``linkedin-weekly-invite-cap`` rule.

Exercises the migration directly (``upgrade`` / ``downgrade``) AND through
``MigrationRunner`` (``apply`` + ``rollback`` paths). Covers:

* Every policy file with an existing ``rules:`` list gets the canonical
  rule appended via ``_policy_io.add_rule_block_text``.
* Re-apply is idempotent — files already carrying the canonical rule
  name are skipped (D74 rule-name-lookup convention).
* Operators who renamed the rule (e.g. ``linkedin-weekly-cap-100``)
  keep their version; the migration logs the operator-deliberate
  divergence + adds the canonical-named rule alongside per D74.
* Dry-run reports affected_count without mutation.
* Refuse-loud on ``ctx.policy_dir`` doesn't exist on disk.
* Refuse-loud on unparseable / non-mapping / missing-rules policy files.
...
* The migration is registered in ``policy.MIGRATIONS`` after policy/0001.
* No file ``version:`` bump (D76: no schema change → no engine SUPPORTED
  set update) — files remain at their pre-migration version.
* No ``migration_event`` ledger emission (policy migrations are ledger-
  silent per ADR-0012 I5).

See ``docs/adr/0020-pillar-c-per-channel-policy-migrations.md`` for the
design rationale.
"""
...
def _make_ctx(
    ...
    *,
    ...
    return MigrationContext(
        ...
        logger=logging.getLogger("test.migrations.policy.0002"),
    )
...
class TestMigrationShape:
    def test_migration_id(self):
        ...
        assert MIGRATION.id == "0002_add_li_invite_weekly_cap"
    ...
    def test_module_constants(self):
        ...
        assert RULE_NAME == "linkedin-weekly-invite-cap"
        assert RULE_TYPE == "budget.window-cap"
        assert RULE_SOURCE == "linkedin_invite"
        assert RULE_BLOCK_WHEN_CHANNEL == "linkedin"
        assert RULE_WINDOW_DAYS == 7
        assert RULE_MAX_UNITS == 100

    def test_description_mentions_linkedin_and_cap(self):
    ...
    def test_migration_registered_in_policy_init(self):
        ...
        Week 7 migration AFTER policy/0001."""
        ...
        assert idx_0001 < idx_0002

    def test_source_matches_pillar_c_week_2_dispatcher_emit(self):
        """Per ADR-0015 D40, Pillar C Week 2's LinkedIn-invite dispatcher
        emits cost_incurred events with source="linkedin_invite" (NOT
        the legacy source="linkedin" from ADR-0008's factory-commented
        shape). The migration's rule MUST match the dispatcher's actual
        emit value — otherwise the rule activates but never fires."""
    ...
    def test_policy_migration_does_not_emit_migration_event(
        ...
        ctx = MigrationContext(
            ...
            policy_dir=policy_dir,
            now=datetime(2026, 5, 22, 12, 0, 0, tzinfo=timezone.utc),
            logger=logging.getLogger("test.policy.0002.no_ledger_emit"),
        )
...
class TestApplyDirect:
    ...
    def test_does_not_bump_version_v2(
        ...
        """Per D75/D76: per-channel rule additions do NOT bump the file
        version. The engine continues to accept the unchanged version."""
    ...
    def test_idempotent_when_operator_renamed_rule(
        ...
        """Per D74: operators who renamed the rule (different name,
        canonical filter shape) — the migration recognizes their version
        + adds the canonical-named rule alongside. The operator's
        explicit choice to rename is respected; the canonical name
        becomes available for downstream tooling that filters on it."""
        policy_with_renamed = _V2_POLICY_BASELINE + (
            "  - name: linkedin-weekly-cap-100\n"
        ...
        assert "linkedin-weekly-cap-100" in rule_names
    ...
    def test_idempotent_when_exact_canonical_name_already_present(
        ...
        uncommented per ADR-0008), the migration skips entirely. The
        operator's version stays as-is (potentially with different
...
class TestRefuseLoud:
    ...
    def test_refuses_rules_null(
        ...
        """Per per-week-review P2-A: `rules: null` (vs `rules: []`) is
        operator-corrupted state. The text-level append helper would
        otherwise corrupt the file by inserting a list-entry after a
        scalar value."""
        _write_policy(
            policy_dir, "bad.yml",
            "version: 2\nrules: null\n",
    ...
    def test_refuses_rules_string(
        ...
        """Per per-week-review P2-A: `rules: some-string` is similarly
        invalid + refuses loud."""
        _write_policy(
            policy_dir, "bad.yml",
            "version: 2\nrules: a-string\n",
        )
    ...
    def test_refuses_rules_mapping(
        ...
        """Per per-week-review P2-A: `rules: {}` (map, not list) is
        similarly invalid + refuses loud."""
        _write_policy(
            policy_dir, "bad.yml",
            "version: 2\nrules: {}\n",
        )
...
# ---------------------------------------------------------------------------
# Per-week-review P2-B: stale-source warning for ADR-0020 §D77 Shape 1
# ---------------------------------------------------------------------------


class TestStaleSourceWarning:
    """Operators who manually uncommented the factory rule before Pillar
    C Week 2 have ``source: linkedin`` (the pre-Week-2 ADR-0008 shape).
    The dispatcher emits ``source="linkedin_invite"`` (per ADR-0015
    D40), so their rule fires on zero events. The migration's name-
    match idempotence skips their file (canonical name present), but
    must emit a WARNING log so the next dispatcher run's logs surface
    the misconfig."""

    def test_warns_on_stale_source_when_canonical_name_present(
        self, policy_dir: Path, state_dir: Path, caplog,
    ):
        import logging
        policy_with_stale = _V2_POLICY_BASELINE + (
            f"  - name: {RULE_NAME}\n"
            "    type: budget.window-cap\n"
            "    block_when:\n"
            "      channel: linkedin\n"
            "    source: linkedin\n"  # STALE — pre-Week-2 shape
            "    window_days: 7\n"
            "    max_units: 100\n"
            "    reason: 'pre-Week-2 shape'\n"
        )
        _write_policy(policy_dir, "cd.yml", policy_with_stale)
        ctx = _make_ctx(policy_dir, state_dir)
        ...
        assert any("stale source" in m for m in warning_messages)
        assert any("linkedin_invite" in m for m in warning_messages)

    def test_no_warning_when_source_already_correct(
        self, policy_dir: Path, state_dir: Path, caplog,
    ):
        """Operators with the Pillar-C-correct shape (source:
        linkedin_invite) get NO warning — their rule is healthy."""
        import logging
        ...
        assert not any("stale source" in m for m in warning_messages)


# ---------------------------------------------------------------------------
# Per-week-review P2-A downgrade companion: rules-not-list refuse on
# downgrade too.
# ---------------------------------------------------------------------------
...
class TestDowngradeRefusesNonListRules:
    def test_downgrade_refuses_rules_null(
        ...
        """Defense-in-depth: downgrade also refuses when `rules:` is
        not a list (mirrors upgrade's P2-A guard)."""
        _write_policy(
            policy_dir, "bad.yml",
            "version: 2\nrules: null\n",
        )
...
class TestDowngrade:
    ...
    def test_downgrade_does_not_remove_renamed_rule(
        ...
        assert "linkedin-weekly-cap-100" in names
...
# ---------------------------------------------------------------------------
#
# Per-week-review P2-C documentation note: the engine's
...
# in this file explicitly import `budget` (and `cooldown` for files
# referencing cooldown rules) inside the test body so the registry
# side-effect happens reliably regardless of test-collection order.
# Weeks 8-11 per-channel policy migration tests follow the same
