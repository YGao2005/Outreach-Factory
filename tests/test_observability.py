...
class TestModuleConstants:
    ...
    def test_event_class_catalog_covers_pillar_a_through_f(self):
        """Every Pillar A-F event class in the foundation ADRs."""
...
class TestTracedStageBehavioralPassthrough:
    ...
    def test_nested_spans_carry_parent_child_relationship(self):
        ...
        provider, exporter = _make_tracer_with_in_memory_exporter()
        tracer = get_tracer(tracer_provider=provider)
        with traced_stage("review", "draft_quality_score",
                          tracer=tracer):
            with traced_stage("draft", "voice_corpus_retrieve",
                              tracer=tracer):
                pass
        spans = exporter.get_finished_spans()
        # Two spans surfaced.
        assert len(spans) == 2
        # Find parent (review) + child (draft) by name.
        by_name = {s.name: s for s in spans}
        parent = by_name["outreach_factory.review.draft_quality_score"]
        child = by_name["outreach_factory.draft.voice_corpus_retrieve"]
...
#   ``person_id`` attribute.
# * ``TestPillarFCallSiteSpanWiring`` — research + review cells:
#   :func:`orchestrator.voice_corpus.retrieve_voice_exemplars` emits
#   the ``outreach_factory.research.voice_corpus_retrieve`` span
#   carrying ``register`` + ``channel`` attributes;
#   :func:`orchestrator.draft_quality.score_draft` emits the
#   ``outreach_factory.review.score_draft`` span carrying ``register``
#   + ``channel`` + ``result_state`` attributes;
#   :func:`orchestrator.reconcile.run_pass_c` emits the
#   ``outreach_factory.review.reconcile_pass_c`` span (Layer 5
#   backstop wrapper per ADR-0049 D262).
...
class TestPillarFCallSiteSpanWiring:
    """ADR-0055 D301 — Pillar F primitives emit spans at call sites.

    Cells: :func:`voice_corpus.retrieve_voice_exemplars` emits the
    research span; :func:`draft_quality.score_draft` emits the review
    span; :func:`reconcile.run_pass_c` emits the review span (Layer 5
    backstop wrapper per ADR-0049 D262).
    """

    def test_score_draft_emits_review_span(self, in_memory_tracer):
        """ADR-0055 D301 — :func:`score_draft` wraps body in
        ``traced_stage("review", "score_draft", ...)``."""
        from orchestrator import draft_quality
        # Empty draft + empty dossier → state="ready" (no claims to
        # uncite). register + channel are closed-enums.
        _ = draft_quality.score_draft(
            draft="",
            dossier="",
            register="cold-pitch",
            channel="email",
        ...
        spans = _spans_named(
            in_memory_tracer,
            "outreach_factory.review.score_draft",
        )
        assert len(spans) == 1, (
            "score_draft MUST emit one review span per call per "
            "ADR-0055 D301"
        )
        assert spans[0].attributes["stage"] == "review"
        assert spans[0].attributes["operation"] == "score_draft"

    def test_score_draft_span_carries_register_and_channel(
        self, in_memory_tracer,
    ):
        """ADR-0055 D301 — the review span carries ``register`` +
        ``channel`` attributes."""
        from orchestrator import draft_quality
        _ = draft_quality.score_draft(
            draft="",
        ...
        assert spans[0].attributes.get("register") == "congrats"
        assert spans[0].attributes.get("channel") == "linkedin-dm"

    def test_reconcile_pass_c_emits_review_span(self, in_memory_tracer, tmp_path):
        """ADR-0055 D301 — :func:`run_pass_c` wraps body in
        ``traced_stage("review", "reconcile_pass_c", ...)`` per the
        Layer 5 backstop integration per ADR-0049 D262."""
...
class TestWeek6PrivacyInvariantPropagation:
    ...
    def test_review_span_has_no_privacy_attrs(self, in_memory_tracer):
        from orchestrator import draft_quality
        _ = draft_quality.score_draft(
            draft="Confidential draft body that MUST NOT leak.",
            dossier="Confidential dossier body that MUST NOT leak.",
            register="cold-pitch",
            channel="email",
        )
        spans = _spans_named(
        ...
        # Also verify the actual confidential bodies do NOT appear
        # anywhere in the attribute values.
        for v in attrs.values():
            v_str = str(v)
            assert "MUST NOT leak" not in v_str, (
                "review span attribute leaked draft/dossier body "
                "content per the privacy invariant"
            )
...
class TestWeek6LegacyStateNoBehavioralImpact:
    ...
    def test_score_draft_state_unchanged_with_span_init(
        self, in_memory_tracer,
    ):
        """ADR-0055 D306 — :func:`score_draft` returns state="ready"
        for empty draft/dossier (no claims to uncite) regardless of
        span init."""
        from orchestrator import draft_quality
        result = draft_quality.score_draft(
            draft="",
            dossier="",
            register="cold-pitch",
            channel="email",
...
# Pillar G Week 10-11 imports per ADR-0058 D319-D324.
from observability import (
...
# ---------------------------------------------------------------------------
# Pillar G Week 10-11 — per-Person observability surface adapter tests
# Per ADR-0058 D319-D324
# ---------------------------------------------------------------------------


def _draft_quality_scored_event(
    *,
    day: int,
    hour: int = 0,
    person_id: str | None = "person-001",
    register: str = "cold-pitch",
    channel: str | None = "email",
    voice_fidelity_score: float = 0.80,
    voice_fidelity_threshold: float = 0.70,
    state: str = "ready",
    draft_hash: str = "sha256:abc",
    exemplar_ids: list[str] | None = None,
    k: int = 5,
    recovered_by: str | None = None,
) -> dict:
    """Build a synthetic ``draft_quality_scored`` event for Week 10-11
    tests. Mirrors :func:`build_draft_quality_scored_payload` shape
    per ADR-0045 D231."""
    ev: dict = {
        ...
        "state": state,
        "exemplar_ids": exemplar_ids if exemplar_ids is not None else [],
        "k": k,
...
def _hallucination_detected_event(
    *,
    day: int,
    hour: int = 0,
    person_id: str | None = "person-001",
    ...
    """Build a synthetic ``hallucination_detected`` event for Week 10-11
    tests. Mirrors :func:`build_hallucination_detected_payload` per
    ADR-0043 D216."""
...
def _reconcile_drift_event(
    *,
    day: int,
    hour: int = 0,
    person_id: str | None = "person-001",
    reason: str = "ready_without_draft_ready_event",
    recovered_by: str | None = None,
) -> dict:
    """Build a synthetic ``reconcile_drift`` event for Week 10-11 tests.
    Mirrors Pass C emit shape per ADR-0011 + ADR-0049 D262."""
    ev: dict = {
        "type": "reconcile_drift",
        "ts": _ts(day, hour=hour),
        "person_id": person_id,
        "reason": reason,
        "_emitted_by": "reconcile",
    }
    if recovered_by is not None:
        ev["_recovered_by"] = recovered_by
    return ev
...
class TestWeek10_11PillarFPersonEventClasses:
    """ADR-0058 D319 — :data:`PILLAR_F_PERSON_EVENT_CLASSES` is the
    closed-set of the FOUR Pillar F event classes the per-Person
    observability surface consumes per ADR-0050 D277."""

    def test_pillar_f_person_event_classes_is_frozenset(self):
        assert isinstance(PILLAR_F_PERSON_EVENT_CLASSES, frozenset)

    def test_pillar_f_person_event_classes_membership(self):
        """The FOUR per-DRAFT Pillar F event classes per ADR-0038 D182:

        * ``voice_exemplar_retrieved`` (Pillar F Week 2 — ADR-0039 D189)
        * ``hallucination_detected`` (Pillar F Week 6 — ADR-0043 D216)
        * ``draft_quality_scored`` (Pillar F Week 8 — ADR-0045 D231)
        * ``draft_ready`` (Pillar F Week 10 — ADR-0047 D246)
        """
        assert PILLAR_F_PERSON_EVENT_CLASSES == frozenset({
    ...
    def test_pillar_f_person_event_classes_cardinality_is_4(self):
        assert len(PILLAR_F_PERSON_EVENT_CLASSES) == 4

    def test_pillar_f_person_event_classes_subset_of_event_class_catalog(self):
        """ADR-0058 D319 — every member is in :data:`EVENT_CLASS_CATALOG`
        (the per-Person primitives walk events recorded under those
        classes)."""
        assert PILLAR_F_PERSON_EVENT_CLASSES.issubset(EVENT_CLASS_CATALOG)


class TestWeek10_11PillarFLayer5DriftReasons:
    """ADR-0058 D321 — :data:`PILLAR_F_LAYER_5_DRIFT_REASONS` is the
    closed-set of the TWO subscribed Layer 5 drift reasons (BOTH legacy
    + new per the Pillar F Week 12 follow-up's reason-precedence drift
    discipline per ADR-0049 §66 P2-2)."""

    def test_pillar_f_layer_5_drift_reasons_is_frozenset(self):
        assert isinstance(PILLAR_F_LAYER_5_DRIFT_REASONS, frozenset)

    def test_pillar_f_layer_5_drift_reasons_includes_both_legacy_and_new(self):
        """ADR-0058 D321 + ADR-0049 §66 P2-2 — the dashboard MUST
        subscribe to BOTH:

        * ``"vault_ahead_of_ledger"`` — the pre-Week-12 reason the
          Layer 5 check PRE-EMPTS whenever ``vault_stage == "ready"``.
        * ``"ready_without_draft_ready_event"`` — the new canonical
          Layer 5 backstop reason per ADR-0049 D262.

        Operators filtering on EITHER reason alone LOSE visibility
        across the post-Week-12 reason-precedence drift; the
        structural commitment is BOTH-must-be-present.
        """
        assert "vault_ahead_of_ledger" in PILLAR_F_LAYER_5_DRIFT_REASONS
        assert (
            "ready_without_draft_ready_event"
            in PILLAR_F_LAYER_5_DRIFT_REASONS
        )

    def test_pillar_f_layer_5_drift_reasons_cardinality_is_2(self):
        """ADR-0058 D321 — exactly TWO subscribed reasons. The third
        ``vault_has_stage_but_ledger_empty`` is Pass C bootstrap drift,
        NOT Layer 5 backstop (operators consume that via OTHER
        dashboards)."""
        assert len(PILLAR_F_LAYER_5_DRIFT_REASONS) == 2

    def test_pillar_f_layer_5_drift_reasons_subset_of_all_drift_reasons(self):
        """ADR-0058 D321 — every subscribed reason MUST be a valid
        :data:`_DRIFT_REASONS` member (no schema drift on the
        dashboard's own constants)."""
        assert PILLAR_F_LAYER_5_DRIFT_REASONS.issubset(
            PILLAR_F_ALL_DRIFT_REASONS_MIRROR,
        )

    def test_pillar_f_layer_5_drift_reasons_excludes_bootstrap_drift(self):
        """ADR-0058 D321 — ``vault_has_stage_but_ledger_empty`` is
        Pass C bootstrap drift; NOT a Layer 5 backstop concern."""
        assert (
            "vault_has_stage_but_ledger_empty"
            not in PILLAR_F_LAYER_5_DRIFT_REASONS
        )


class TestWeek10_11PillarFMirrorsParity:
    """ADR-0058 D322 — the per-pillar mirror constants
    (:data:`PILLAR_F_REGISTERS_MIRROR`,
    :data:`PILLAR_F_CLAIM_TYPES_MIRROR`,
    :data:`PILLAR_F_ALL_DRIFT_REASONS_MIRROR`) match upstream closed-
    sets at test time. The mirror constants decouple
    :mod:`orchestrator.observability` from the upstream modules at
    runtime; the test-time parity check IS the regression-barrier."""

    def test_pillar_f_registers_mirror_matches_voice_corpus_registers(self):
        from orchestrator.voice_corpus import REGISTERS
        assert PILLAR_F_REGISTERS_MIRROR == REGISTERS

    def test_pillar_f_claim_types_mirror_matches_draft_quality_claim_types(self):
        from orchestrator.draft_quality import CLAIM_TYPES
        assert PILLAR_F_CLAIM_TYPES_MIRROR == CLAIM_TYPES

    def test_pillar_f_all_drift_reasons_mirror_matches_reconcile_drift_reasons(self):
        from orchestrator.reconcile import _DRIFT_REASONS
        assert PILLAR_F_ALL_DRIFT_REASONS_MIRROR == _DRIFT_REASONS

    def test_pillar_f_registers_mirror_is_frozenset(self):
        assert isinstance(PILLAR_F_REGISTERS_MIRROR, frozenset)

    def test_pillar_f_claim_types_mirror_is_frozenset(self):
        assert isinstance(PILLAR_F_CLAIM_TYPES_MIRROR, frozenset)

    def test_pillar_f_all_drift_reasons_mirror_is_frozenset(self):
        assert isinstance(PILLAR_F_ALL_DRIFT_REASONS_MIRROR, frozenset)


class TestWeek10_11DiagnosticKindsClosedSetExtension:
    """ADR-0058 D322 — :data:`_DIAGNOSTIC_KINDS` extends from 3 to 6
    values at Pillar G Week 10-11. The closed-set discipline carries
    forward per ADR-0051 D279 + ADR-0057 D317."""

    def test_diagnostic_kinds_cardinality_is_6(self):
        assert len(_DIAGNOSTIC_KINDS) == 6

    def test_diagnostic_kinds_membership(self):
        assert _DIAGNOSTIC_KINDS == frozenset({
            "uncatalogued",
            "missing_ts",
    ...
    def test_diagnostic_kinds_includes_pillar_f_register_kind(self):
        assert "pillar_f_register_uncatalogued" in _DIAGNOSTIC_KINDS

    def test_diagnostic_kinds_includes_pillar_f_claim_type_kind(self):
        assert "pillar_f_claim_type_uncatalogued" in _DIAGNOSTIC_KINDS

    def test_diagnostic_kinds_includes_pillar_f_drift_reason_kind(self):
        assert "pillar_f_drift_reason_uncatalogued" in _DIAGNOSTIC_KINDS

    def test_diagnostic_kinds_preserves_prior_kinds(self):
        """ADR-0058 D322 — the Week 10-11 extension is additive; the
        Week 2 + Week 9 kinds preserve verbatim."""
        for prior in (
            "uncatalogued", "missing_ts", "cost_source_uncatalogued",
        ):
            assert prior in _DIAGNOSTIC_KINDS


class TestWeek10_11PersonRegisterFidelitySnapshotDataclass:
    """ADR-0058 D319 — :class:`PersonRegisterFidelitySnapshot` is the
    frozen dataclass for the per-(person_id, register) voice-fidelity
    snapshot."""

    def test_dataclass_is_frozen(self):
        snap = PersonRegisterFidelitySnapshot(
            person_id="p1", register="cold-pitch", channel="email",
            event_count=1, total_fidelity_score=0.8,
            min_fidelity_score=0.8, max_fidelity_score=0.8,
            ready_count=1, refused_count=0,
        )
        with pytest.raises(Exception):
            snap.event_count = 99  # type: ignore[misc]

    def test_dataclass_field_set(self):
        """ADR-0058 D319 + privacy invariant — fields are COUNTS +
        SCORES; NEVER draft body / exemplar bodies / per-claim trace.
        """
        snap = PersonRegisterFidelitySnapshot(
            person_id="p1", register="cold-pitch", channel="email",
            event_count=3, total_fidelity_score=2.4,
            min_fidelity_score=0.75, max_fidelity_score=0.90,
            ready_count=2, refused_count=1,
            oldest_ts="2026-05-20T00:00:00.000Z",
            newest_ts="2026-05-22T00:00:00.000Z",
        )
        assert snap.person_id == "p1"
        assert snap.register == "cold-pitch"
        assert snap.channel == "email"
        assert snap.event_count == 3
        assert snap.total_fidelity_score == 2.4
        assert snap.min_fidelity_score == 0.75
        assert snap.max_fidelity_score == 0.90
        assert snap.ready_count == 2
        assert snap.refused_count == 1
        assert snap.oldest_ts == "2026-05-20T00:00:00.000Z"
        assert snap.newest_ts == "2026-05-22T00:00:00.000Z"


class TestWeek10_11PersonClaimTypeHallucinationSnapshotDataclass:
    """ADR-0058 D320 — :class:`PersonClaimTypeHallucinationSnapshot`
    dataclass shape."""

    def test_dataclass_is_frozen(self):
        snap = PersonClaimTypeHallucinationSnapshot(
            person_id="p1", claim_type="named_entity",
            channel="email", register="cold-pitch",
            event_count=1, uncited_claim_count=2,
        )
        with pytest.raises(Exception):
            snap.event_count = 99  # type: ignore[misc]
    ...
    def test_dataclass_field_set(self):
        snap = PersonClaimTypeHallucinationSnapshot(
            person_id="p1", claim_type="named_entity",
            channel="email", register="cold-pitch",
            event_count=2, uncited_claim_count=5,
            oldest_ts="2026-05-20T00:00:00.000Z",
            newest_ts="2026-05-22T00:00:00.000Z",
        )
        assert snap.person_id == "p1"
        assert snap.claim_type == "named_entity"
        assert snap.channel == "email"
        assert snap.register == "cold-pitch"
        assert snap.event_count == 2
        assert snap.uncited_claim_count == 5


class TestWeek10_11PersonLayer5DriftSnapshotDataclass:
    """ADR-0058 D321 — :class:`PersonLayer5DriftSnapshot` dataclass
    shape."""

    def test_dataclass_is_frozen(self):
        snap = PersonLayer5DriftSnapshot(
            person_id="p1", reason="ready_without_draft_ready_event",
            event_count=1,
        )
        with pytest.raises(Exception):
            snap.event_count = 99  # type: ignore[misc]
    ...
    def test_dataclass_field_set(self):
        snap = PersonLayer5DriftSnapshot(
            person_id="p1", reason="vault_ahead_of_ledger",
            event_count=3,
            oldest_ts="2026-05-20T00:00:00.000Z",
            newest_ts="2026-05-22T00:00:00.000Z",
        )
        assert snap.person_id == "p1"
        assert snap.reason == "vault_ahead_of_ledger"
        assert snap.event_count == 3
        assert snap.oldest_ts == "2026-05-20T00:00:00.000Z"
        assert snap.newest_ts == "2026-05-22T00:00:00.000Z"


class TestWeek10_11CollectPerPersonRegisterFidelityAggregation:
    """ADR-0058 D319 — :func:`collect_per_person_register_fidelity_snapshots`
    aggregates per ``(person_id, register)`` pair correctly."""

    def test_single_person_single_register_single_event(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1", register="cold-pitch",
                voice_fidelity_score=0.85, state="ready",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
            led2, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        )
        assert len(snaps) == 1
        assert snaps[0].person_id == "p1"
        assert snaps[0].register == "cold-pitch"
        assert snaps[0].event_count == 1
        assert snaps[0].total_fidelity_score == pytest.approx(0.85)
        assert snaps[0].min_fidelity_score == pytest.approx(0.85)
        assert snaps[0].max_fidelity_score == pytest.approx(0.85)
        assert snaps[0].ready_count == 1
        assert snaps[0].refused_count == 0

    def test_single_person_single_register_multiple_events(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=20, person_id="p1", register="cold-pitch",
                voice_fidelity_score=0.70, state="refused",
            ),
            _draft_quality_scored_event(
                day=22, person_id="p1", register="cold-pitch",
                voice_fidelity_score=0.90, state="ready",
            ),
            _draft_quality_scored_event(
                day=24, person_id="p1", register="cold-pitch",
                voice_fidelity_score=0.85, state="ready",
            ),
        ])
        led2 = Ledger(led_dir)
        ...
        snap = snaps[0]
        assert snap.event_count == 3
        assert snap.total_fidelity_score == pytest.approx(2.45)
        assert snap.min_fidelity_score == pytest.approx(0.70)
        assert snap.max_fidelity_score == pytest.approx(0.90)
        assert snap.ready_count == 2
        assert snap.refused_count == 1
        assert snap.oldest_ts == _ts(20)
        assert snap.newest_ts == _ts(24)

    def test_multiple_persons_same_register(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1", register="cold-pitch",
            ...
            _draft_quality_scored_event(
                day=22, person_id="p2", register="cold-pitch",
                voice_fidelity_score=0.90,
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
        ...
        assert len(snaps) == 2
        # Sorted by person_id alphabetically.
        assert snaps[0].person_id == "p1"
        assert snaps[1].person_id == "p2"

    def test_single_person_multiple_registers(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1", register="cold-pitch",
            ),
            _draft_quality_scored_event(
                day=22, person_id="p1", register="reply",
            ),
            _draft_quality_scored_event(
                day=22, person_id="p1", register="congrats",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
        ...
        assert len(snaps) == 3
        # Sorted by register alphabetically within same person.
        assert [s.register for s in snaps] == [
            "cold-pitch", "congrats", "reply",
        ]

    def test_person_id_none_handled(self, led_dir, led):
        """ADR-0045 D231 — ``person_id=None`` is permitted for ad-hoc
        validation events; the primitive surfaces the ad-hoc bucket
        with ``person_id=None`` snapshot."""
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id=None, register="cold-pitch",
            ),
            _draft_quality_scored_event(
                day=22, person_id="p1", register="cold-pitch",
            ),
        ...
        assert len(snaps) == 2
        # None-person sorts BEFORE Person-stamped per the
        # ``(person_id_or_empty, ...)`` sort key.
        assert snaps[0].person_id is None
        assert snaps[1].person_id == "p1"


class TestWeek10_11CollectPerPersonRegisterFidelityWindowFilter:
    """ADR-0058 D319 — window filter on ``ts >= since_iso``."""

    def test_event_before_since_excluded(self, led_dir, led):
        # since is 2026-05-15; event on day 10 is before.
        _direct_write(led_dir, [
            _draft_quality_scored_event(day=10, person_id="p1"),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
            led2, since=datetime(2026, 5, 15, tzinfo=timezone.utc),
            now=NOW_2026_05_25,
        )
        assert snaps == []

    def test_event_after_since_included(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(day=20, person_id="p1"),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
            led2, since=datetime(2026, 5, 15, tzinfo=timezone.utc),
...
class TestWeek10_11CollectPerPersonRegisterFidelityR032SyntheticExclusion:
    """ADR-0058 D319 + ADR-0056 D311 + ADR-0057 D314 — events with
    ``_recovered_by`` are EXCLUDED from the per-Person fidelity
    aggregation (R032 mitigation)."""

    def test_recovered_by_event_excluded(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1",
                recovered_by="migration_2026-05-20",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
    ...
    def test_non_recovered_event_included(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1",
            ),
    ...
    def test_mix_of_recovered_and_not(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1",
                recovered_by="migration_2026-05-20",
            ),
            _draft_quality_scored_event(day=22, person_id="p1"),
            _draft_quality_scored_event(
                day=23, person_id="p1",
                recovered_by="backfill_2026-05-21",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
        ...
        # Only the non-_recovered_by event aggregates.
        assert len(snaps) == 1
        assert snaps[0].event_count == 1


class TestWeek10_11CollectPerPersonRegisterFidelityChannelInvariant:
    """ADR-0058 D319 + ADR-0014 D33 — channel-on-every-event invariant.
    Snapshot ``channel`` is homogeneous single channel value if every
    in-window event for the pair carries the same channel; ``None``
    otherwise."""

    def test_homogeneous_single_channel_surfaces(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1", channel="email",
            ),
            _draft_quality_scored_event(
                day=23, person_id="p1", channel="email",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
    ...
    def test_heterogeneous_channels_surface_none(self, led_dir, led):
        _direct_write(led_dir, [
            ...
            _draft_quality_scored_event(
                day=23, person_id="p1", channel="linkedin-dm",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
    ...
    def test_no_channel_surfaces_none(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1", channel=None,
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
...
class TestWeek10_11CollectPerPersonRegisterFidelityDeterministicOrdering:
    """ADR-0058 D324 + ADR-0031 D140 + ADR-0051 D280 — snapshots
    sorted by ``(person_id_or_empty, register)`` for deterministic
    output."""

    def test_sorted_by_person_id_then_register(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p3", register="reply",
            ),
            _draft_quality_scored_event(
                day=22, person_id="p1", register="reply",
            ),
        ...
        assert [(s.person_id, s.register) for s in snaps] == [
            ("p1", "cold-pitch"),
            ("p1", "reply"),
            ("p2", "cold-pitch"),
            ("p3", "reply"),
...
class TestWeek10_11CollectPerPersonRegisterFidelityDeterministicClock:
    """ADR-0058 D324 — ``now`` kwarg controls the diagnostic emit ts."""

    def test_now_stamps_diagnostic_ts(self, led_dir, led):
        # Trigger a diagnostic by using an unknown register.
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1", register="unknown-register",
            ),
        ])
        led2 = Ledger(led_dir)
        anchor = datetime(2026, 5, 25, 14, 30, 0, tzinfo=timezone.utc)
        collect_per_person_register_fidelity_snapshots(
            led2, since=SINCE_2026_05_01, now=anchor,
        )
        diag = [
            e for e in led2.all_events()
            if e.type == "observability_class_uncatalogued"
            and e.get("kind") == "pillar_f_register_uncatalogued"
        ]
        assert len(diag) == 1
        assert diag[0].ts == "2026-05-25T14:30:00.000Z"


class TestWeek10_11CollectPerPersonRegisterFidelityRegisterUncataloguedDiagnostic:
    """ADR-0058 D322 — unknown ``register`` triggers
    ``pillar_f_register_uncatalogued`` diagnostic emit (at-most-ONE per
    call per R034 mitigation)."""

    def test_one_unknown_register_triggers_emit(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1", register="formal-intro",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_register_fidelity_snapshots(
        ...
        assert len(diag) == 1
        assert diag[0].get("offending_value") == "formal-intro"
        assert diag[0].get("count") == 1
        assert diag[0].get("_emitted_by") == "observability"

    def test_multiple_unknown_registers_aggregate_to_one_emit(
        self, led_dir, led,
    ):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
            ...
            _draft_quality_scored_event(
                day=23, person_id="p2", register="another-unknown",
            ),
        ])
        led2 = Ledger(led_dir)
        collect_per_person_register_fidelity_snapshots(
        ...
        # At-most-ONE per kind per call per R034.
        assert len(diag) == 1
        assert diag[0].get("count") == 2

    def test_known_register_no_diagnostic(self, led_dir, led):
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1", register="cold-pitch",
            ),
...
class TestWeek10_11CollectPerPersonClaimTypeHallucinationAggregation:
    """ADR-0058 D320 — per-(person_id, claim_type) aggregation walks
    each ``uncited_claims`` entry."""

    def test_single_event_single_claim(self, led_dir, led):
        _direct_write(led_dir, [
            _hallucination_detected_event(
                day=22, person_id="p1",
                uncited_claims=[{
                    "claim_type": "named_entity",
                    "claim_text": "Acme",
        ...
        led2 = Ledger(led_dir)
        snaps = collect_per_person_claim_type_hallucination_snapshots(
            led2, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        )
        assert len(snaps) == 1
        assert snaps[0].person_id == "p1"
        assert snaps[0].claim_type == "named_entity"
        assert snaps[0].event_count == 1
        assert snaps[0].uncited_claim_count == 1

    def test_one_event_multiple_claims_same_type(self, led_dir, led):
        """One event with multiple uncited claims of the same type
        contributes ``event_count=1`` + ``uncited_claim_count=<n>``."""
        _direct_write(led_dir, [
            _hallucination_detected_event(
                day=22, person_id="p1",
                uncited_claims=[
        ...
        assert len(snaps) == 1
        assert snaps[0].event_count == 1
        assert snaps[0].uncited_claim_count == 3

    def test_one_event_multiple_claim_types(self, led_dir, led):
        """One event with claims of different types contributes one
        snapshot per type."""
        _direct_write(led_dir, [
            _hallucination_detected_event(
                day=22, person_id="p1",
                uncited_claims=[
                    ...
                    {
                        "claim_type": "date_reference",
                        "claim_text": "March 2024",
                        "citation_anchor": None,
                    },
                ],
            ),
        ...
        assert len(snaps) == 2
        # Sorted by (person_id, claim_type).
        assert snaps[0].claim_type == "date_reference"
        assert snaps[1].claim_type == "named_entity"
        for snap in snaps:
            assert snap.event_count == 1
            assert snap.uncited_claim_count == 1

    def test_multiple_events_same_person_same_claim_type(self, led_dir, led):
        _direct_write(led_dir, [
            _hallucination_detected_event(
                day=20, person_id="p1",
                uncited_claims=[{
                    "claim_type": "you_phrase",
                    "claim_text": "you mentioned",
                    "citation_anchor": None,
                }],
            ),
            _hallucination_detected_event(
        ...
        assert len(snaps) == 1
        assert snaps[0].event_count == 2
        assert snaps[0].uncited_claim_count == 2


class TestWeek10_11CollectPerPersonClaimTypeHallucinationClaimTypeUncataloguedDiagnostic:
    """ADR-0058 D322 — unknown ``claim_type`` triggers
    ``pillar_f_claim_type_uncatalogued`` diagnostic emit."""

    def test_one_unknown_claim_type_triggers_emit(self, led_dir, led):
        _direct_write(led_dir, [
            _hallucination_detected_event(
                day=22, person_id="p1",
                uncited_claims=[{
                    "claim_type": "newfangled_pattern",
                    "claim_text": "X",
                    "citation_anchor": None,
                }],
            ),
        ...
        diag = [
            e for e in led2.all_events()
            if e.type == "observability_class_uncatalogued"
            and e.get("kind") == "pillar_f_claim_type_uncatalogued"
        ]
        assert len(diag) == 1
        assert diag[0].get("offending_value") == "newfangled_pattern"
        assert diag[0].get("count") == 1
        assert diag[0].get("_emitted_by") == "observability"

    def test_mixed_known_and_unknown_claim_types(self, led_dir, led):
        _direct_write(led_dir, [
            _hallucination_detected_event(
                day=22, person_id="p1",
                uncited_claims=[
        ...
        # Only the known type contributes a snapshot.
        assert len(snaps) == 1
        assert snaps[0].claim_type == "named_entity"
        # Unknown triggers diagnostic.
        diag = [
            e for e in led2.all_events()
            if e.type == "observability_class_uncatalogued"
            and e.get("kind") == "pillar_f_claim_type_uncatalogued"
    ...
    def test_at_most_one_per_call(self, led_dir, led):
        _direct_write(led_dir, [
            _hallucination_detected_event(
                day=22, person_id="p1",
                uncited_claims=[
...
class TestWeek10_11CollectPerPersonClaimTypeHallucinationR032SyntheticExclusion:
    """ADR-0058 D320 + R032 — events with ``_recovered_by`` excluded."""
...
class TestWeek10_11CollectPerPersonClaimTypeHallucinationChannelInvariant:
    """ADR-0058 D320 + ADR-0014 D33 — channel + register homogeneity."""
    ...
    def test_heterogeneous_channel_surfaces_none(self, led_dir, led):
        _direct_write(led_dir, [
            _hallucination_detected_event(
                day=22, person_id="p1", channel="email",
            ),
    ...
    def test_homogeneous_register_surfaces(self, led_dir, led):
        _direct_write(led_dir, [
            _hallucination_detected_event(
                day=22, person_id="p1", register="cold-pitch",
            ),
            _hallucination_detected_event(
                day=23, person_id="p1", register="cold-pitch",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_claim_type_hallucination_snapshots(
    ...
    def test_heterogeneous_register_surfaces_none(self, led_dir, led):
        _direct_write(led_dir, [
            _hallucination_detected_event(
                day=22, person_id="p1", register="cold-pitch",
            ),
            _hallucination_detected_event(
                day=23, person_id="p1", register="reply",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_claim_type_hallucination_snapshots(
        ...
        assert snaps[0].register is None


class TestWeek10_11CollectPerPersonClaimTypeHallucinationWindowFilter:
    """ADR-0058 D320 — window filter."""

    def test_event_before_since_excluded(self, led_dir, led):
        _direct_write(led_dir, [
            _hallucination_detected_event(day=10, person_id="p1"),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_claim_type_hallucination_snapshots(
            led2, since=datetime(2026, 5, 15, tzinfo=timezone.utc),
...
class TestWeek10_11CollectPerPersonLayer5DriftAggregation:
    """ADR-0058 D321 — per-(person_id, reason) Layer 5 drift
    aggregation. The default subscribed reasons are BOTH
    ``vault_ahead_of_ledger`` AND ``ready_without_draft_ready_event``
    per the Pillar F Week 12 follow-up's reason-precedence drift
    discipline."""

    def test_single_person_single_reason(self, led_dir, led):
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p1",
                reason="ready_without_draft_ready_event",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_layer_5_drift_snapshots(
            led2, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        )
        assert len(snaps) == 1
        assert snaps[0].person_id == "p1"
        assert snaps[0].reason == "ready_without_draft_ready_event"
        assert snaps[0].event_count == 1

    def test_single_person_both_reasons(self, led_dir, led):
        """ADR-0058 D321 + ADR-0049 §66 P2-2 — BOTH legacy
        (vault_ahead_of_ledger) AND new (ready_without_draft_ready_event)
        reasons surface in the default subscribed set."""
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p1",
                reason="vault_ahead_of_ledger",
    ...
    def test_multiple_events_same_person_same_reason(self, led_dir, led):
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=20, person_id="p1",
                reason="ready_without_draft_ready_event",
    ...
    def test_multiple_persons(self, led_dir, led):
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p1",
                reason="ready_without_draft_ready_event",
...
class TestWeek10_11CollectPerPersonLayer5DriftReasonPrecedenceDriftPattern:
    """ADR-0058 D321 + ADR-0049 §66 P2-2 — the Pillar F Week 12 follow-
    up's NEW legacy-state-vs-new-defense-layer reason-precedence drift
    pattern. The per-Person Layer 5 dashboard MUST subscribe to BOTH
    reasons; the regression-barrier test pins the structural
    commitment."""

    def test_default_subscribed_reasons_include_both(self, led_dir, led):
        """The default PILLAR_F_LAYER_5_DRIFT_REASONS includes BOTH
        legacy + new reason values. Operators using the framework
        default see BOTH columns in the dashboard.
        """
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p1",
        ...
        led2 = Ledger(led_dir)
        # Use default subscribed_reasons (no kwarg override).
        snaps = collect_per_person_layer_5_drift_snapshots(
            led2, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        )
        reasons = {s.reason for s in snaps}
        assert "vault_ahead_of_ledger" in reasons
        assert "ready_without_draft_ready_event" in reasons

    def test_caller_subscribing_only_new_reason_loses_legacy_visibility(
        self, led_dir, led,
    ):
        """ADR-0058 D321 — the Pillar F Week 12 follow-up pattern: if
        an operator subscribes ONLY to the new reason, they lose
        visibility of refusals that the Layer 5 check PRE-EMPTS from
        the legacy reason. This test documents the failure mode that
        the default closed-set protects against.
        """
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p1",
        ...
        snaps = collect_per_person_layer_5_drift_snapshots(
            led2, since=SINCE_2026_05_01, now=NOW_2026_05_25,
            subscribed_reasons=frozenset({
                "ready_without_draft_ready_event",
            }),
        )
        # Operators see ZERO drift; the legacy reason is silently
        # filtered out (this is the bug the default closed-set
        # protects against).
        assert snaps == []


class TestWeek10_11CollectPerPersonLayer5DriftSilentSkipOnNonSubscribed:
    """ADR-0058 D321 — known-but-not-subscribed reasons are SILENTLY
    SKIPPED (no diagnostic emit). The ``vault_has_stage_but_ledger_empty``
    reason is Pass C bootstrap drift, NOT a Layer 5 backstop concern."""

    def test_vault_has_stage_but_ledger_empty_silently_skipped(
        self, led_dir, led,
    ):
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p1",
                reason="vault_has_stage_but_ledger_empty",
            ),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_layer_5_drift_snapshots(
        ...
        assert snaps == []
        # No diagnostic — it's a known reason; just not subscribed.
        diag = [
            e for e in led2.all_events()
            if e.type == "observability_class_uncatalogued"
        ]
...
class TestWeek10_11CollectPerPersonLayer5DriftReasonUncataloguedDiagnostic:
    """ADR-0058 D322 — unknown ``reason`` (NOT in
    :data:`PILLAR_F_ALL_DRIFT_REASONS_MIRROR`) triggers
    ``pillar_f_drift_reason_uncatalogued`` diagnostic emit (schema
    drift detection)."""

    def test_unknown_reason_triggers_emit(self, led_dir, led):
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p1",
        ...
        diag = [
            e for e in led2.all_events()
            if e.type == "observability_class_uncatalogued"
            and e.get("kind") == "pillar_f_drift_reason_uncatalogued"
        ]
        assert len(diag) == 1
        assert diag[0].get("offending_value") == "newly_invented_reason"
        assert diag[0].get("_emitted_by") == "observability"

    def test_at_most_one_per_call(self, led_dir, led):
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p1", reason="invented_1",
            ),
            _reconcile_drift_event(
                day=23, person_id="p2", reason="invented_2",
            ),
        ])
        led2 = Ledger(led_dir)
        collect_per_person_layer_5_drift_snapshots(
...
class TestWeek10_11CollectPerPersonLayer5DriftR032SyntheticExclusion:
    """ADR-0058 D321 + R032 — events with ``_recovered_by`` excluded."""
...
class TestWeek10_11CollectPerPersonLayer5DriftWindowFilter:
    """ADR-0058 D321 — window filter."""

    def test_event_before_since_excluded(self, led_dir, led):
        _direct_write(led_dir, [
            _reconcile_drift_event(day=10, person_id="p1"),
        ])
        led2 = Ledger(led_dir)
        snaps = collect_per_person_layer_5_drift_snapshots(
            led2, since=datetime(2026, 5, 15, tzinfo=timezone.utc),
...
class TestWeek10_11CollectPerPersonLayer5DriftDeterministicOrdering:
    """ADR-0058 D324 — snapshots sorted by ``(person_id, reason)``."""

    def test_sorted_by_person_id_then_reason(self, led_dir, led):
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p2",
                reason="ready_without_draft_ready_event",
        ...
        assert [(s.person_id, s.reason) for s in snaps] == [
            ("p1", "ready_without_draft_ready_event"),
            ("p1", "vault_ahead_of_ledger"),
            ("p2", "ready_without_draft_ready_event"),
        ]


class TestWeek10_11AllPersonPrimitivesEmptyAndUnrelated:
    """ADR-0058 D319-D321 — empty ledger + unrelated events produce
    empty snapshot list + zero diagnostic emits."""

    def test_register_fidelity_empty_ledger(self, led_dir, led):
        snaps = collect_per_person_register_fidelity_snapshots(
            led, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        )
        assert snaps == []
        assert led.all_events() == []

    def test_claim_type_hallucination_empty_ledger(self, led_dir, led):
        snaps = collect_per_person_claim_type_hallucination_snapshots(
            led, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        )
        assert snaps == []
    ...
    def test_layer_5_drift_empty_ledger(self, led_dir, led):
        snaps = collect_per_person_layer_5_drift_snapshots(
            led, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        )
        assert snaps == []
    ...
    def test_unrelated_events_ignored_by_all_primitives(self, led_dir, led):
        """ADR-0058 D319-D321 — non-Pillar-F events (e.g., send_intent)
        are ignored; the primitives are scoped to specific event class
        types."""
        _direct_write(led_dir, [{
            "type": "send_intent",
            "ts": _ts(22),
            "person_id": "p1",
            "channel": "email",
            "_emitted_by": "send_queued",
        }])
        led2 = Ledger(led_dir)
        assert collect_per_person_register_fidelity_snapshots(
            led2, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        ) == []
        led3 = Ledger(led_dir)
        assert collect_per_person_claim_type_hallucination_snapshots(
            led3, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        ) == []
        led4 = Ledger(led_dir)
        assert collect_per_person_layer_5_drift_snapshots(
            led4, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        ) == []


class TestWeek10_11AllPersonPrimitivesPrivacyInvariant:
    """ADR-0058 D323 + I8 + ADR-0038 D182 + ADR-0050 D276(b) — the per-
    Person dashboard surfaces COUNTS + SCORES only; NEVER draft body,
    exemplar bodies, dossier content, per-claim trace text, query
    content, or source_list. The dataclass shape IS the structural
    commitment."""
    ...
    def test_person_register_fidelity_snapshot_has_no_body_fields(self):
        ...
        forbidden = {
            "draft_body", "exemplar_body", "exemplar_bodies",
            "dossier_body", "claim_text", "query", "query_text",
            "source_list", "raw_body",
        }
        assert forbidden.isdisjoint(set(fields))

    def test_person_claim_type_hallucination_snapshot_has_no_body_fields(self):
        fields = (
            PersonClaimTypeHallucinationSnapshot
        ...
        forbidden = {
            "draft_body", "exemplar_body", "exemplar_bodies",
            "dossier_body", "claim_text", "query", "query_text",
            "source_list", "raw_body", "uncited_claims",
        }
        assert forbidden.isdisjoint(set(fields))

    def test_person_layer_5_drift_snapshot_has_no_body_fields(self):
        fields = PersonLayer5DriftSnapshot.__dataclass_fields__.keys()
        forbidden = {
            "draft_body", "exemplar_body", "exemplar_bodies",
            "dossier_body", "claim_text", "query", "query_text",
            "source_list", "raw_body", "vault_stage", "ledger_stage",
        }
        assert forbidden.isdisjoint(set(fields))


class TestWeek10_11PillarFEventsNotUncatalogued:
    """ADR-0058 D319 + R031 — events from the Pillar G Week 10-11
    diagnostic kinds do NOT trigger recursive uncatalogued diagnostics
    on the per-event-class observability primitive (per ADR-0050 D272's
    permissive-aggregate-with-explicit-enumeration via
    ``expected_classes | OBSERVABILITY_NEW_EVENT_CLASSES``)."""

    def test_pillar_f_diagnostic_emit_not_uncatalogued_on_event_class(
        self, led_dir, led,
    ):
        """A pillar_f_register_uncatalogued diagnostic emit (which uses
        the existing ``observability_class_uncatalogued`` event class)
        is ALREADY in ``OBSERVABILITY_NEW_EVENT_CLASSES``; subsequent
        ``collect_event_class_snapshots`` calls do NOT flag it as
        uncatalogued."""
        # Trigger a diagnostic.
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1", register="unknown",
            ),
        ])
        led2 = Ledger(led_dir)
        collect_per_person_register_fidelity_snapshots(
        ...
        # Now call event-class primitive — the
        # observability_class_uncatalogued event is in
        # OBSERVABILITY_NEW_EVENT_CLASSES so it does NOT trigger
        # an "uncatalogued" diagnostic on the per-event-class
        # primitive.
        led3 = Ledger(led_dir)
        collect_event_class_snapshots(
            led3, since=SINCE_2026_05_01, now=NOW_2026_05_25,
        ...
        led4 = Ledger(led_dir)
        uncat = [
            e for e in led4.all_events()
            if e.type == "observability_class_uncatalogued"
            and e.get("kind") == "uncatalogued"
        ]
        assert uncat == []


class TestWeek10_11GrafanaPerPersonDashboardYaml:
    """ADR-0058 D324 — the Grafana per-Person dashboard YAML exists +
    has the required panels."""

    def test_dashboard_yaml_exists(self):
        from pathlib import Path as _Path
        repo_root = _Path(__file__).resolve().parent.parent
        dashboard = (
            repo_root / "infra" / "grafana" / "dashboards"
            / "per_person.yml"
        )
        assert dashboard.exists()

    def test_dashboard_yaml_renders_pillar_f_event_classes(self):
        """ADR-0058 D324 — the Grafana per-Person dashboard YAML
        references the four Pillar F event classes."""
        from pathlib import Path as _Path
        repo_root = _Path(__file__).resolve().parent.parent
        dashboard = (
            repo_root / "infra" / "grafana" / "dashboards"
        ...
        # All four Pillar F event classes referenced in PromQL.
        for ec in PILLAR_F_PERSON_EVENT_CLASSES:
            assert ec in content, f"event_class {ec!r} missing"
        # Layer 5 drift reason referenced.
        assert "reconcile_drift" in content

    def test_dashboard_yaml_documents_both_layer_5_reasons(self):
        """ADR-0058 D324 + ADR-0049 §66 P2-2 — the Layer 5 panel's
        description must document BOTH legacy + new reasons per the
        Pillar F Week 12 follow-up's reason-precedence drift
        discipline."""
        from pathlib import Path as _Path
        repo_root = _Path(__file__).resolve().parent.parent
        dashboard = (
        ...
        content = dashboard.read_text()
        assert "vault_ahead_of_ledger" in content
        assert "ready_without_draft_ready_event" in content


class TestWeek10_11ModuleDocstringDriftDiscipline:
    """ADR-0058 — the module-level docstring drift discipline (NOW
    THIRTEEN consecutive weeks at Pillar G Week 10-11; Pillar F W8-W12
    + Pillar G W2-W6 + W7-W8 + W9 + W10-11) requires the observability
    module's docstring names the Week 10-11 deliverables + ADR-0058."""

    def test_module_docstring_names_week_10_11(self):
        doc = observability.__doc__ or ""
        assert "Week 10-11" in doc
        assert "ADR-0058" in doc
        assert "collect_per_person_register_fidelity_snapshots" in doc


class TestWeek10_11BehavioralPassthrough:
    """ADR-0058 — the behavioral-passthrough-not-signature-only
    discipline (NOW ELEVEN consecutive weeks at Pillar G Week 10-11;
    Pillar F W8-W11 + Pillar G W3-W6 + W7-W8 + W9 + W10-11). Per-call
    kwargs (``expected_registers``, ``expected_claim_types``,
    ``subscribed_reasons``) ACTUALLY flow through to the aggregation
    path."""

    def test_expected_registers_kwarg_flows_to_diagnostic(self, led_dir, led):
        """A caller passing a restricted ``expected_registers`` sees
        diagnostic emits for events using framework-known registers
        that are NOT in the restricted set."""
        _direct_write(led_dir, [
            _draft_quality_scored_event(
                day=22, person_id="p1", register="cold-pitch",
            ),
        ...
        collect_per_person_register_fidelity_snapshots(
            led2, since=SINCE_2026_05_01, now=NOW_2026_05_25,
            expected_registers=frozenset({"reply"}),  # restricted
        )
        diag = [
            e for e in led2.all_events()
            if e.type == "observability_class_uncatalogued"
        ...
        assert len(diag) == 1
        assert diag[0].get("offending_value") == "cold-pitch"

    def test_expected_claim_types_kwarg_flows_to_diagnostic(
        self, led_dir, led,
    ):
        _direct_write(led_dir, [
            _hallucination_detected_event(
        ...
        collect_per_person_claim_type_hallucination_snapshots(
            led2, since=SINCE_2026_05_01, now=NOW_2026_05_25,
            expected_claim_types=frozenset({"date_reference"}),
        )
        diag = [
            e for e in led2.all_events()
            if e.type == "observability_class_uncatalogued"
        ...
        assert len(diag) == 1
        assert diag[0].get("offending_value") == "named_entity"

    def test_subscribed_reasons_kwarg_flows_to_filter(self, led_dir, led):
        """A caller passing a restricted ``subscribed_reasons`` sees
        ONLY drift events with reasons in the restricted set."""
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p1",
                reason="vault_ahead_of_ledger",
        ...
        # Only the new reason surfaces.
        assert len(snaps) == 1
        assert snaps[0].reason == "ready_without_draft_ready_event"

    def test_known_reasons_kwarg_flows_to_diagnostic(self, led_dir, led):
        """A caller passing a restricted ``known_reasons`` sees
        diagnostic emits for events using reasons NOT in the
        restricted set (even if those reasons are in the framework
        default :data:`PILLAR_F_ALL_DRIFT_REASONS_MIRROR`)."""
        _direct_write(led_dir, [
            _reconcile_drift_event(
                day=22, person_id="p1",
                reason="vault_ahead_of_ledger",
        ...
        assert len(diag) == 1
        assert diag[0].get("offending_value") == "vault_ahead_of_ledger"
