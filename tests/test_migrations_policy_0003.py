"""Tests for policy migration 0003 — add ``linkedin-weekly-dm-cap`` rule.

Pillar C Week 8's per-channel policy migration. Mirrors Week 7's
``policy/0002_add_li_invite_weekly_cap`` shape per ADR-0020 D72's
per-week trajectory + ADR-0021's D79-D83. The structural shape is
identical modulo the channel filter / source value / max_units
default; the per-week-review-driven hardening of Week 7 (the
...
* Every policy file with an existing ``rules:`` list gets the canonical
  ``linkedin-weekly-dm-cap`` rule appended via ``_policy_io.add_rule_block_text``.
* Re-apply is idempotent — files already carrying the canonical rule
  ...
  ADR-0020).
* Operators who renamed the rule (e.g. ``linkedin-weekly-dm-cap-50``)
  keep their version; the migration adds the canonical-named rule
  alongside per D74.
...
* The migration is registered in ``policy.MIGRATIONS`` after policy/0002.
* No file ``version:`` bump (D75/D76 inherited from ADR-0020 — content-
  additive migration, no engine SUPPORTED set extension).
* No ``migration_event`` ledger emission (policy migrations are ledger-
  silent per ADR-0012 I5).
* Source filter matches Pillar C Week 3 dispatcher emit
  (``source="linkedin_dm"`` per ADR-0016 D43).
* **NO stale-source warning path.** Unlike Week 7's policy/0002 (which
  detects ``source: linkedin`` from pre-Pillar-C-Week-2 operators per
  ADR-0020 §D77 Shape 1 + warns at WARNING level), Week 8 has no
  pre-Pillar-C-Week-3 factory-commented LinkedIn DM rule for operators
  to have stale-copied. The ADR-0008 / ADR-0015 staleness path is
  invite-specific. Per ADR-0021 D81. This invariant is pinned by
...
See ``docs/adr/0021-pillar-c-linkedin-dm-weekly-cap.md`` for the
design rationale.
"""
...
def _make_ctx(
    ...
    *,
    ...
    return MigrationContext(
        ...
        logger=logging.getLogger("test.migrations.policy.0003"),
    )
...
class TestMigrationShape:
    def test_migration_id(self):
        ...
        assert MIGRATION.id == "0003_add_li_dm_weekly_cap"
    ...
    def test_module_constants(self):
        ...
        assert RULE_NAME == "linkedin-weekly-dm-cap"
        assert RULE_TYPE == "budget.window-cap"
        assert RULE_SOURCE == "linkedin_dm"
        assert RULE_BLOCK_WHEN_CHANNEL == "linkedin"
        assert RULE_WINDOW_DAYS == 7
        # ADR-0021 D79: conservative half-of-invite default. Operators
        # tune up (Premium / Sales Navigator) or down (warm-up phase).
        assert RULE_MAX_UNITS == 50

    def test_description_mentions_linkedin_dm_and_cap(self):
    ...
    def test_migration_registered_in_policy_init(self):
        ...
        Week 8 migration AFTER policy/0002."""
        ...
        assert idx_0001 < idx_0002 < idx_0003

    def test_source_matches_pillar_c_week_3_dispatcher_emit(self):
        """Per ADR-0016 D43, Pillar C Week 3's LinkedIn-DM dispatcher
        emits cost_incurred events with source="linkedin_dm". The
    ...
    def test_source_distinct_from_li_invite(self):
        """ADR-0015 D40's split-source convention: linkedin_dm is a
        distinct source from linkedin_invite. The rule's cap applies to
        DMs ONLY; invites are gated by Week 7's separate rule."""
        from orchestrator.migrations.policy.migration_0002_add_li_invite_weekly_cap import (
            RULE_SOURCE as INVITE_SOURCE,
        )
        assert RULE_SOURCE != INVITE_SOURCE
    ...
    def test_policy_migration_does_not_emit_migration_event(
        ...
        Same posture as policy/0001 + policy/0002."""
        ...
        ctx = MigrationContext(
            ...
            policy_dir=policy_dir,
            now=datetime(2026, 5, 22, 12, 0, 0, tzinfo=timezone.utc),
            logger=logging.getLogger("test.policy.0003.no_ledger_emit"),
        )
...
class TestApplyDirect:
    ...
    def test_does_not_bump_version_v2(
        ...
        """Per D75/D76 inherited from ADR-0020: per-channel rule additions
        do NOT bump the file version. The engine continues to accept
    ...
    def test_idempotent_when_operator_renamed_rule(
        ...
        """Per D74 (inherited from ADR-0020): operators who renamed the
        rule (different name, canonical filter shape) — the migration
        recognizes their version + adds the canonical-named rule
        alongside. The operator's explicit choice to rename is
        respected; the canonical name becomes available for downstream
        tooling that filters on it."""
        policy_with_renamed = _V2_POLICY_BASELINE + (
            "  - name: linkedin-weekly-dm-cap-50\n"
        ...
        assert "linkedin-weekly-dm-cap-50" in rule_names
    ...
    def test_coexists_with_invite_cap_rule(
        ...
        policy/0002) has the DM cap rule added without conflict —
        per-channel split-source convention (ADR-0015 D40 + ADR-0021
        D79) means both rules coexist + independently throttle."""
        ...
        assert RULE_NAME in rule_names
        # Sources are distinct per ADR-0015 D40.
        sources = {r["name"]: r.get("source") for r in data["rules"]}
        assert sources["linkedin-weekly-invite-cap"] == "linkedin_invite"
        assert sources[RULE_NAME] == "linkedin_dm"
...
# ---------------------------------------------------------------------------
# NO stale-source warning path — D81 (ADR-0021)
# ---------------------------------------------------------------------------


class TestNoStaleSourceWarning:
    """ADR-0021 D81: Unlike Week 7's policy/0002 (which warns when
    ...
    ADR-0008's factory comment), Week 8 has NO analogous staleness path.

    Reason: the LinkedIn DM dispatcher (ADR-0016) shipped AFTER ADR-0008
    + ADR-0015 D40's split-source convention established. There has
    never been a factory-shipped ``linkedin-weekly-dm-cap`` rule with a
    stale ``source: linkedin`` field — the canonical source from day
    one is ``linkedin_dm``. No operator could have copied a stale
    ...
    structural difference between the two migrations.
    """

    def test_no_warning_when_canonical_rule_has_source_linkedin(
        self, policy_dir: Path, state_dir: Path, caplog,
    ):
        """If an operator hand-wrote a ``linkedin-weekly-dm-cap`` rule
        with ``source: linkedin`` (the legacy ADR-0008 shape — which
        was never a factory shape for DMs), the migration MUST skip
        without emitting a stale-source warning. The operator's
        deliberate choice of source is respected; no doctor-like
        nagging.
    ...
    def test_no_warning_when_canonical_rule_has_source_linkedin_invite(
        ...
        """If an operator's ``linkedin-weekly-dm-cap`` rule has
        ``source: linkedin_invite`` (a likely copy-paste mistake from
        Week 7's invite-cap rule), the migration still skips without
    ...
    def test_no_warning_when_canonical_rule_is_correct(
        ...
        linkedin_dm) get NO warning — their rule is healthy."""
...
class TestDowngradeRefusesNonListRules:
    def test_downgrade_refuses_rules_null(
        ...
        downgrade also refuses when `rules:` is not a list (mirrors
        upgrade's guard)."""
        _write_policy(
            policy_dir, "bad.yml",
            "version: 2\nrules: null\n",
...
class TestDowngrade:
    ...
    def test_downgrade_does_not_remove_renamed_rule(
        ...
        assert "linkedin-weekly-dm-cap-50" in names
    ...
    def test_downgrade_does_not_remove_invite_cap_rule(
        ...
        """Defense-in-depth: downgrading Week 8's DM cap must NOT touch
        Week 7's invite cap rule. The two are name-distinct + the
        per-channel split-source convention (ADR-0015 D40) means each
        rule's downgrade is independent."""
        policy_with_invite = _V2_POLICY_BASELINE + (
            "  - name: linkedin-weekly-invite-cap\n"
            "    type: budget.window-cap\n"
            "    block_when:\n"
        ...
        data = yaml.safe_load(f.read_text(encoding="utf-8"))
        names = [r["name"] for r in data["rules"]]
        # DM cap removed; invite cap preserved.
        assert RULE_NAME not in names
...
# forward: the engine's `load_rules_from_yaml` consults `RULE_REGISTRY`,
# which is populated at import-time by each rule-class module. Engine-
# integration tests in this file explicitly import `budget` (and
# `cooldown` for files referencing cooldown rules) inside the test body
# so the registry side-effect happens reliably regardless of test-
# collection order. Weeks 9-11 per-channel policy migration tests
# follow the same pattern; do NOT skip the imports.
...
class TestRealFactoryTemplateRoundTrip:
    ...
    def test_factory_template_has_commented_rule_12c(
        self,
    ):
        """ADR-0021 D80: the factory template ships a commented Rule 12c
        documenting the LinkedIn DM cap shape for new operators. The
        rule mirrors Rule 12b's structure modulo channel-source /
        max_units / reason — Pillar C's per-channel symmetry."""
        text = FACTORY_TEMPLATE.read_text(encoding="utf-8")
        # The Rule 12c block-comment header is present.
        assert "Rule 12c" in text
        ...
        # Week 3 dispatcher emit (per ADR-0016 D43).
        assert "source: linkedin_dm" in text
        # The commented default is 50 per ADR-0021 D79.
